...
def normalize_company_name_for_similarity(s: str) -> str:
    """
    거래처/retail_user 비교용 전처리:
    - 회사 접미사(株式会社/（株）/（주） 등)만 제거
    - 그 외 괄호/기호는 원문을 최대한 보존 (무지성으로 괄호 문자를 제거하지 않음)
    - 공백/전각 공백 제거 후 순수 텍스트만 반환
    ...
    # 회사 접미사 토큰만 제거
    # - 株式会社
    # - (株), (株式), (주) : fullwidth/halfwidth 괄호가 있어도 NFKC로 '(' ')' 로 통일된 상태로 처리
    t = t.replace("株式会社", "")
    t = re.sub(r"\(\s*(?:株|株式)\s*\)", "", t)
    t = re.sub(r"\(\s*주\s*\)", "", t)

    # 괄호/기호는 최대한 보존하고, 유사도에 방해되는 일부 구두점만 제거
    for c in ",，.．、":
        t = t.replace(c, "")
